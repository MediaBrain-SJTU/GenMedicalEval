...
1.**大量的院内数据**：UniMedicalEval拥有大量的患者治疗记录，这是之前的医疗评测数据集所不具备的独特优势。患者治疗记录提供了更为丰富且复杂的数据，真实反映了医疗领域的复杂现实，包括各种疾病、症状、病史、治疗方案以及患者反应等多方面信息。此外，患者治疗记录通常包含非结构化的信息，如医生笔记、实验室报告和影像资料，因此要求模型具备处理多种数据类型的能力。因此，患者治疗记录为评估医疗LLMs提供了更贴近实际医疗实践的复杂场景，使我们能够更全面地评估模型的实用性和有效性。



2.**安全性与伦理**：UniMedicalEval深入探讨了医学领域的安全性与伦理问题，并构建了四个关键方面的评测数据集，包括医疗反事实、毒害伦理、歧视校验和患者知情权。这一举措填补了医疗领域在安全性与伦理评测数据集方面的明显不足。这样一来，UniMedicalEval能够更好地协助用户建立对医疗LLMs的信任，确保这些医疗LLMs不仅提高了医疗服务的质量，还保护了患者的权益。


3.**特色的评价指标**：UniMedicalEval提出了一种创新的生成式评测指标，其基于结构化回复抽取和医学术语的对齐。与以往评估方法中的多项选择准确率和GPT-4评分等指标不同，UniMedicalEval能够从知识性的角度评估医疗LLMs在回答开放式问题时的表现。这一新方法使得UniMedicalEval能够更全面地综合评估医疗LLMs的能力。
