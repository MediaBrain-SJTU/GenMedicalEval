...
1.建立从基础知识能力到临床应用能力到安全性鲁棒性等的分层评价体系，保证评测全面性

2.建立、完善评测数据的构建基准，数据质量，保证评测可靠性

3.设计针对生成式大语言模型的特色评价指标，保证评测区分性
...
为了对模型的基础知识能力、场景应用能力与规范服从能力进行全面、可靠的评估，UniMedicalEval设计了针对生成式大语言模型的多样化评测指标。
